    ...
    <v-layout wrap>
      <v-flex xs12 sm12 md9>
        <v-card class="elevation-1 ma-1 pa-2">
          <v-text-field v-model="form.title" prepend-icon="edit" label="Tag title" required></v-text-field>
...
export default {
  ...
  methods: {
    ...
    update(status){
      ...
      this.$axios.patch(`/tags/${this.$route.params.slug}`, this.form).then(res => {
        this.snackbar = true;
        this.$router.push(`/dashboard/tags/edit/${res.data.data.slug}`);
  ...
  mounted() {
       this.$axios.get(`/tags/${this.$route.params.slug}`).then(res => {
        const tag = res.data.data;

        if (tag.image != null) {
          this.imageId = tag.image.id;
          this.imageUrl = tag.image.size.medium;
          this.imageFile = tag.image.size.medium;
          this.imageName = tag.image.title;
          this.form.image = tag.image.id;
        }
        this.form.id = tag.id;
