    ...
    <v-layout wrap>
      <v-flex xs12 sm12 md9>
        <v-card class="elevation-1 ma-1 pa-2">
          <v-text-field v-model="form.title" prepend-icon="edit" label="Region title" required></v-text-field>
      ...
      <v-flex xs12 sm12 md3>
        <v-card class="elevation-1 ma-1">
          ...
          <v-divider></v-divider>
          <v-card-actions>
            <v-btn @click.prevent="update(3)" class="flat">Draft</v-btn>
            <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    //on remove chips
    removeRegion(item) {
      this.post.regions.splice(this.post.regions.indexOf(item), 1);
      this.post.regions = [...this.post.regions];
    },
    ...
    update(status){
      ...
      this.$axios.patch(`/regions/${this.$route.params.slug}`, this.form).then(res => {
        this.snackbar = true;
        this.$router.push(`/dashboard/regions/edit/${res.data.data.slug}`);
  ...
  mounted() {
       this.$axios.get(`/regions/${this.$route.params.slug}`).then(res => {
        const region = res.data.data;

        if (region.image != null) {
          this.imageId = region.image.id;
          this.imageUrl = region.image.size.medium;
          this.imageFile = region.image.size.medium;
          this.imageName = region.image.title;
          this.form.image = region.image.id;
        }
        this.form.id = region.id;
        this.form.title = region.title;
        this.form.description = region.description;
