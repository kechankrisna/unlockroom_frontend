    ...
    <v-layout wrap>
      <v-flex xs12 sm12 md9>
        <v-card class="elevation-1 ma-1 pa-2">
          ...
          <v-card-text>
            ...
            <v-img class="elevation-5" :src="cover.url" :ratio="16/9" v-if="cover.url" max-height="400" />
      ...
      <v-flex xs12 sm12 md3>
        <v-card class="elevation-1 ma-1">
          ...
          <v-divider></v-divider>
          <v-card-actions>
            <v-btn @click.prevent="update(3)">SAVE</v-btn>
            <v-spacer></v-spacer>
            <v-btn @click.prevent="update(4)" class="primary">Update</v-btn>
          </v-card-actions>
        </v-card>
 ...
 */
    ...
    update(status) {
      ...
      this.$axios
        .patch(`/venues/${this.$route.params.slug}`, this.form)
        .then(res => {
          this.snackbar = true;
          this.$router.push(`/dashboard/venues/edit/${res.data.data.slug}`);
        })
        .catch(err => {
          console.log(err);
        });
  ...
  mounted() {
    ...
    this.$axios.get(`/venues/${this.$route.params.slug}`).then(res => {
        const venue = res.data.data;

        if (venue.image != null) {
          this.image.id = venue.image.id;
          this.image.url = venue.image.size.medium;
          this.image.file = venue.image.size.medium;
          this.image.name = venue.image.title;
          this.form.image = venue.image.id;
        }
        if (venue.cover != null) {
          this.cover.id = venue.cover.id;
          this.cover.url = venue.cover.size.large;
          this.cover.file = venue.cover.size.large;
          this.cover.name = venue.cover.title;
          this.form.cover = venue.cover.id;
        }
        ...
        this.form = {
          ...
          deposit_rate: venue.deposit_rate,
          tax_rate: venue.tax_rate,
          ...
          cover: venue.cover.id,
          galleries: [...venue.galleries.map(gallery => {
            return gallery.id;
          })]
        };

      this.galleries = venue.galleries;
