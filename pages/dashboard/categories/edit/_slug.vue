    ...
    <v-layout wrap>
      <v-flex xs12 sm12 md9>
        <v-card class="elevation-1 ma-1 pa-2">
          <v-text-field v-model="form.title" prepend-icon="edit" label="Category title" required></v-text-field>
...
export default {
  ...
  methods: {
    ...
    update(status){
      ...
      this.$axios.patch(`/categories/${this.$route.params.slug}`, this.form).then(res => {
        this.snackbar = true;
        this.$router.push(`/dashboard/categories/edit/${res.data.data.slug}`);
  ...
  mounted() {
       this.$axios.get(`/categories/${this.$route.params.slug}`).then(res => {
        const category = res.data.data;

        if (category.image != null) {
          this.imageId = category.image.id;
          this.imageUrl = category.image.size.medium;
          this.imageFile = category.image.size.medium;
          this.imageName = category.image.title;
